...
public class AppJDBCTemplate implements AppDAO {
	...
	@Override
	public AccountDetails validateAccountWithToken(final DecodedJWT token) {
		String query = String.format("select * from authAccounts where username=?");
		...
		List<AccountDetails> details = this.jdbcTemplate.query(query, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				// TODO Auto-generated method stub
				ps.setString(1, token.getSubject());
